...
- The selected Actor is `harvestapi/linkedin-company-employees`. R21 supports only the current `Short ($4 per 1k)` input mode. The Actor pricing table currently says `$0.003` per Basic profile, while the input schema label implies `$0.004`; until an account receipt resolves that discrepancy, settlement uses `$0.004` per returned profile plus `$0.020` per all-at-once Actor start. The live Actor metadata also declares a `$0.05` minimum `maxTotalChargeUsd`, so quotes reserve at least that raw provider ceiling even when a small run's expected event cost is lower. The contract version is `harvestapi-linkedin-company-employees-basic-min-0.05-2026-08-22`; a changed actor, mode, price, minimum cap, or resolved lower billing proof requires a new reviewed version.
- One operation accepts exactly one frozen LinkedIn company URL, uses `companyBatchMode=all_at_once`, and returns at most 25 profiles. A 2026-08-22 owner-only run proved that a multi-company Short-mode response can echo the submitted batch without identifying which company produced each row. The single-company boundary is therefore part of plan schema v2: every accepted row must echo that sole URL in `_meta.query.currentCompanies`; a missing, different, or multi-company echo is ambiguous and persists no person. The quote reserves the fixed start event plus the maximum profile events, and the reservation-derived `maxTotalChargeUsd` is sent to Apify. A definitive empty result charges only the start; returned rows charge the start plus every provider-returned profile, including rows the parser cannot safely bind.
...
| R21-C - owner-only golden motion | In progress | 2026-08-22 | The first run proved the `$0.05` Actor minimum; the second completed for `$0.029` but its three multi-company rows were safely withheld and parked for operator reconciliation. Plan schema v2 narrows the retry to one query-echo-bound company before the final no-send golden run. |

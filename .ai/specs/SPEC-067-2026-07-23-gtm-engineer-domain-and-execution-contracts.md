...
- After a model invocation, the route writes one content-free, operation-keyed local telemetry receipt and awaits an idempotent canonical `ai_usage` receipt attributed to the represented Noli user and organization. A canonical write failure returns HTTP 503 and cannot be converted into a successful AI draft or an unmetered template fallback.

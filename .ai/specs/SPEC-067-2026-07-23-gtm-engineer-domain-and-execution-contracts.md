...
**Date:** 2026-07-23 PDT
**Status:** Production-enabled for paid Noli organizations. Governed B2B sourcing, company-to-person resolution, enrichment, approval-bound email execution, mailbox ingestion, replies, suppression, deletion, and canonical credit metering are live behind their independent fail-closed controls. B2C demand-opportunity research is also customer-enabled under SPEC-069, but consumer participation/outreach remains manual-only. Auto-refill, LeadMagic, and Bouncer remain off. Only contract-pinned DataForSEO and Apify capabilities may run through quote/confirm/reconcile boundaries.
...
### 1.0 Current production truth (supersedes historical dark-release language)

As of 2026-09-02, `GTM_ENGINEER_ENABLED`, `GTM_EXECUTION_ENABLED`, and `GTM_MAILBOX_INGESTION_ENABLED` are true in production. GTM is included in every paid plan, including the one-seat Solo plan through that user's organization credit pool. Execution being enabled is not blanket send authority: campaign approval still freezes the exact recipient, sender, content, postal footer, schedule, and unsubscribe contract; suppression and mailbox health are rechecked at claim time. `GTM_AUTO_REFILL_ENABLED` remains false.

The July B2B-only statement above records the original contract boundary. SPEC-069 additively extends customer research to governed US consumer-demand opportunities while permanently retaining `manual_only` consumer outreach. It does not authorize automated consumer email, posting, joining, registration, calling, or messaging.
...
- The August 29 Starter transition supersedes only the account-tier price material: BRONZE retains the `$0.001` start and prices each emitted verification row at `$0.0027`, frozen as `automation-lab-email-enrichment-0.1.49-bronze-0.001-start-0.0027-per-row-2026-08-29`. The one-run `$0.01` provider cap, conservative verification semantics, and actor build remain unchanged. `GTM_APIFY_ACCOUNT_TIER=BRONZE` is now an independent global gate, so code and price-version deployment cannot activate the adapter while the authenticated account still reports FREE or later moves to another tier.
...
### 48.7 Fail-closed GTM AI allowance and metering

- GTM model operations consume the same organization-wide monthly credit pool as the rest of Noli. A solo paid organization is a one-seat organization and therefore receives one seat's pooled monthly allowance; it does not require a separate GTM wallet.
- Every GTM model route requires a server-resolved CRM organization linked to Noli Core, an available canonical allowance read, and an exact operation idempotency key before contacting Gemini. Allowance or mapping outages return typed `ai_metering_unavailable` with HTTP 503; exhausted allowance remains `ai_allowance` with HTTP 402.
- After every model invocation, the route writes one content-free local telemetry receipt and awaits one canonical `ai_usage` receipt attributed to the represented Noli user and organization. Each receipt derives a stable invocation key from the request operation key plus its call index, so multi-step generation charges every actual call while a repeated canonical write remains idempotent. A canonical write failure returns HTTP 503 and cannot be converted into a successful AI draft or an unmetered template fallback.
- Legacy non-GTM CRM AI surfaces retain their prior best-effort helper by default. The strict contract is opt-in at the four customer-facing GTM model boundaries: voice derivation, campaign message regeneration, AI reply drafting, and manual consumer-outreach drafting.
...
- When that follow-up draft exists for a new workspace, the CRM main dashboard presents it as a reviewable first-value artifact. The explicit use action opens the email composer with the subject and body loaded; it never supplies a recipient or sends without the customer acting.
...
- 2026-08-25: Made the seeded CRM follow-up draft visible from the new-workspace dashboard and loadable into the review composer without choosing a recipient or sending.
- 2026-08-31: R89 closes moving-default build drift in the two active legacy HarvestAPI paths. LinkedIn post comments now invoke exact build `0.0.30`; profile enrichment invokes exact build `0.0.131`; both receipts retain `actor_build`; and any admitted source without a reviewed build fails closed before provider contact. The authenticated Starter/BRONZE verifier now proves exact pinned-build availability and the effective rate card for every active Apify actor instead of requiring a mutable `latest` tag to remain equal to an old pin. This is contract hardening only and authorizes no additional provider call, recipient, send, or customer consumer release.
- 2026-09-02: Closed GTM's inherited fail-open AI allowance/metering boundary. All four GTM model routes now require a linked Noli organization, an available canonical allowance read, a stable operation key, and an awaited idempotent Noli Core usage receipt attributed to the represented user; multi-step generation records one receipt per actual model invocation, and a solo plan uses its one-seat organization pool. Metering outages return typed 503 and no AI draft is reported as successful.
- 2026-09-02: Reconciled the release record with production. GTM is customer-enabled for paid organizations; execution and mailbox ingestion are live behind exact approval, postal-address, sender, suppression, and health gates; auto-refill remains off. A Starter/BRONZE Full-mode company-to-person run produced one current, unambiguously bound decision-maker for `$0.028` / `14,000` credits, and the subsequent profile/verification waterfall charged `6,850` credits while honestly retaining an `unknown` deliverability result. No provider-sourced person was contacted.

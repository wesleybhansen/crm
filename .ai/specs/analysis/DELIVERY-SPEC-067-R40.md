...
R46 deployed that replacement contract in CRM merge `1eaa8399aa65f70efea060b62c9cac3f74ce01d3` and ran one bounded owner-only quote/confirm validation against completed research run `73021bf0-67e2-4cb2-a9c8-50e7b3be046e`. The quote covered exactly one company, at most three profiles, and a 25,000-credit ceiling under plan hash `b659680f17ccfc680cc310f044ad049320cef7f9540a1b2834c901faa06b5e42`. Apify run `TG65yvxP2EazYJUYz` received the pinned `Full ($8 per 1k)` input on build `0.0.157`, but the account is on Apify's `FREE` tier and the run ended with `free user run limit exceeded`. Its finalized receipt charged one `short-profile` event, no `actor-start` or Full event, and `$0.003` total. That event was outside the frozen Full-mode contract, so CRM did not read or expose the dataset and created no person, relation, or contact point. Canonical operation `4bbd01d5-45e5-4750-9985-7edd811dc0b7` was explicitly reconciled once to `partially_charged` at 1,500 credits from the provider's terminal receipt. No further decision-maker call is authorized until the account can execute the reviewed Full-mode contract and a fresh quote confirms it.

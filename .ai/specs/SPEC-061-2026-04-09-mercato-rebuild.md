...
- **2026-08-14 (greenfield adoption repair)** — Added the bounded generator-owned legacy-table adoption option and used it to generate the missing email campaign/recipient/message schema. Verified all enabled migrations on a fresh disposable PostgreSQL database, a clean second migration pass, and an email-only no-diff generation pass.

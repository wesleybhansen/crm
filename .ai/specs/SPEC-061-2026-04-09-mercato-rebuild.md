...
2. **Legacy-table adoption must remain generator-owned.** A greenfield database correctly causes MikroORM to generate `CREATE TABLE`, while an upgraded database may already contain the same table from the frozen setup SQL. Use `yarn mercato db generate --adopt-existing-tables=<comma-separated-table-names>` for that bounded adoption case. The generator validates each table name and adds `IF NOT EXISTS` only to generated table/index creation for the named tables. Never hand-write the schema or seed/restore frozen `setup-tables.sql`; reconcile the module snapshot for any already-applied columns or indexes and prove both a fresh migration and a no-diff second run.
...
- **2026-08-14 (greenfield adoption repair)** — Added the bounded generator-owned legacy-table adoption option and used it to generate missing email, billing, landing-page, and webhook schema for already-declared entities. Fresh initialization now applies every enabled migration, handles optional workflow/search metadata safely, rebuilds only enabled query-index modules, and completes without frozen setup SQL; a second migration pass has no pending work.

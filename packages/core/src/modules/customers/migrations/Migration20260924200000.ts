...
export class Migration20260924200000 extends Migration {
  override async up(): Promise<void> {
    this.addSql(globalUniqueIndexSql({ table: 'surveys', column: 'slug', index: 'surveys_slug_global_uniq' }))
    this.addSql(globalUniqueIndexSql({ table: 'chat_widgets', column: 'slug', index: 'chat_widgets_slug_global_uniq', where: 'slug IS NOT NULL' }))
    this.addSql(globalUniqueIndexSql({ table: 'events', column: 'slug', index: 'events_slug_global_uniq', where: 'deleted_at IS NULL' }))
    this.addSql(globalUniqueIndexSql({ table: 'events', column: 'kiosk_token', index: 'events_kiosk_token_global_uniq', where: 'kiosk_token IS NOT NULL AND deleted_at IS NULL' }))
    this.addSql(globalUniqueIndexSql({ table: 'affiliates', column: 'affiliate_code', index: 'affiliates_code_global_uniq' }))
  }

  override async down(): Promise<void> {
    this.addSql(`DROP INDEX IF EXISTS "surveys_slug_global_uniq";`)
    this.addSql(`DROP INDEX IF EXISTS "chat_widgets_slug_global_uniq";`)
    this.addSql(`DROP INDEX IF EXISTS "events_slug_global_uniq";`)
    this.addSql(`DROP INDEX IF EXISTS "events_kiosk_token_global_uniq";`)
    this.addSql(`DROP INDEX IF EXISTS "affiliates_code_global_uniq";`)
  }
}

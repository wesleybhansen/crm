...
- `TENANT_KMS_PROVIDER` – `derived|vault` (default `derived`). `derived` derives a per-tenant key from the data key below; `vault` opts back in to HashiCorp Vault. The legacy name `TENANT_DATA_KMS` is still honoured.
- `TENANT_DATA_ENCRYPTION_KEY` – the tenant data key. Generate with `openssl rand -base64 32`.
- `TENANT_DATA_ENCRYPTION_FALLBACK_KEY` – legacy name for the same key, read only when `TENANT_DATA_ENCRYPTION_KEY` is unset, with one startup warning. `AUTH_SECRET` and `NEXTAUTH_SECRET` are never used for this: deriving the data key from the session secret re-keys every encrypted row the day that secret rotates. In production, neither key variable set means the process refuses to boot rather than writing plaintext.
- `VAULT_ADDR`, `VAULT_TOKEN` – only consulted when `TENANT_KMS_PROVIDER=vault`. Example: `https://vault.example.com`.
- `VAULT_KV_PATH` – KV v2 mount for tenant keys (default `secret/data`).

Envelopes are written as `iv:ct:tag:v2:<keyId>`, where `<keyId>` fingerprints the active key. Older `v1` envelopes still read. A `v2` envelope opened with a different key fails with a typed error naming both key ids instead of leaving ciphertext on the screen.

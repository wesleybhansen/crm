...
- **Key source** – `TENANT_KMS_PROVIDER` selects it and defaults to `derived`: per-tenant keys are PBKDF2-derived from `TENANT_DATA_ENCRYPTION_KEY` (or the legacy `TENANT_DATA_ENCRYPTION_FALLBACK_KEY`). `vault` is opt in only. Vault was primary once, and a sealed Vault silently moved every tenant onto a different key, so it is never consulted unless asked for by name. In production with neither key variable set the process refuses to boot instead of degrading to a noop KMS.
...
- `TENANT_KMS_PROVIDER` – `derived` (default) or `vault`. Legacy name: `TENANT_DATA_KMS`.
- `TENANT_DATA_ENCRYPTION_KEY` – the tenant data key, then `TENANT_DATA_ENCRYPTION_FALLBACK_KEY` as the legacy name for it. `AUTH_SECRET`/`NEXTAUTH_SECRET` are never accepted here.
- `VAULT_ADDR`, `VAULT_TOKEN` – only read when `TENANT_KMS_PROVIDER=vault`.

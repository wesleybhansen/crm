...
import {
  Plus, X, Loader2, GitBranch, Play, Pause, Trash2, Mail, Clock,
...
export default function SequencesPage() {
  ...
  async function openDetail(id: string) {
    ...
    const [seqRes, enrollRes] = await Promise.all([
      ...
      fetch(`/api/sequences/${id}/enrollments`, { credentials: 'include' }).then(r => r.json()),
    ])
    if (seqRes.ok) setSelectedSequence(seqRes.data)
  ...
  async function enrollContact() {
    if (!enrollContactId.trim() || !selectedId) return
    setEnrolling(true)
    const res = await fetch(`/api/sequences/${selectedId}/enroll`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include',
      body: JSON.stringify({ contactId: enrollContactId }),
    })
    const data = await res.json()
    if (data.ok) { setEnrollContactId(''); setShowEnroll(false); openDetail(selectedId) }
  ...
  async function aiDraftStep(idx: number) {
    ...
    const res = await fetch('/api/ai/draft-email', {
      ...
      body: JSON.stringify({
        ...
        purpose: 'follow-up',
        context: `Sequence: ${name}. Step ${idx + 1} of ${steps.length}. ${description || ''}`,
      }),
    })
    const data = await res.json()
    if (data.ok) {
          ...
          <div className="rounded-lg border bg-card p-5 space-y-4">
            ...
            <div className="grid gap-3">
              ...
              {triggerType === 'tag_added' && (
                <div>
                  <label className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider block mb-1">Tag Name</label>
                  <Input value={triggerConfig.tagSlug || ''} onChange={e => setTriggerConfig({ tagSlug: e.target.value })}
                    placeholder="e.g. new-lead" className="h-9 text-sm" />
                </div>
              )}
              {triggerType === 'deal_stage_changed' && (
                <div>
                  <label className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider block mb-1">Stage Name</label>
                  <Input value={triggerConfig.stage || ''} onChange={e => setTriggerConfig({ stage: e.target.value })}
                    placeholder="e.g. Won" className="h-9 text-sm" />
                </div>
              )}
            </div>
          </div>
                ...
                return (
                  <div key={idx} className="rounded-lg border p-4">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      {step.stepType === 'email' && (
                        <Button type="button" variant="outline" size="sm" onClick={() => aiDraftStep(idx)} className="h-7 text-[10px] px-2">
                          <Sparkles className="size-3 mr-1" /> AI Draft
                        </Button>
                      )}
                      {steps.length > 1 && (
      ...
      <div className="p-6 max-w-4xl mx-auto">
        ...
        {showEnroll && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setShowEnroll(false)}>
            <div className="bg-card rounded-lg border shadow-lg p-5 w-96" onClick={e => e.stopPropagation()}>
              <h3 className="text-sm font-semibold mb-3">Enroll Contact</h3>
              <p className="text-xs text-muted-foreground mb-3">Paste the contact's ID to enroll them in this sequence.</p>
              <Input value={enrollContactId} onChange={e => setEnrollContactId(e.target.value)}
                placeholder="Contact ID (UUID)" className="h-9 text-sm mb-3" autoFocus />
              <div className="flex justify-end gap-2">
                <Button type="button" variant="outline" size="sm" onClick={() => setShowEnroll(false)}>Cancel</Button>
                <Button type="button" size="sm" onClick={enrollContact} disabled={enrolling || !enrollContactId.trim()}>
                  {enrolling ? <Loader2 className="size-3 animate-spin mr-1" /> : <UserPlus className="size-3 mr-1" />} Enroll
                </Button>
              </div>
            </div>
          </div>

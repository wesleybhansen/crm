...
export class Migration20260924200000_calendar extends Migration {
  override async up(): Promise<void> {
    this.addSql(globalUniqueIndexSql({ table: 'booking_pages', column: 'slug', index: 'booking_pages_slug_global_uniq' }))
    this.addSql(globalUniqueIndexSql({ table: 'bookings', column: 'confirmation_token', index: 'bookings_confirmation_token_global_uniq', where: 'confirmation_token IS NOT NULL' }))
  }

  override async down(): Promise<void> {
    this.addSql(`DROP INDEX IF EXISTS "booking_pages_slug_global_uniq";`)
    this.addSql(`DROP INDEX IF EXISTS "bookings_confirmation_token_global_uniq";`)
  }
}

...
import {
  ...
  apifyRedditApiOpportunityEnabled,
...
export function sourceAdapterRegistry(): Record<string, SourceAdapter> {
  ...
  if (apifyRedditApiOpportunityEnabled()) {
    const redditApiOpportunities = createApifyRedditApiOpportunityAdapter()
    registry[redditApiOpportunities.descriptor.adapter_id] = redditApiOpportunities
  }
  ...
  if (apifyEventbriteOpportunityEnabled()) {
    const eventbriteOpportunities = createApifyEventbriteOpportunityAdapter()
    registry[eventbriteOpportunities.descriptor.adapter_id] = eventbriteOpportunities
  }

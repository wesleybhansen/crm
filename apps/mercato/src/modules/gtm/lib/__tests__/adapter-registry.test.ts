...
describe('adapter registry environment boundaries', () => {
  ...
  it('registers Eventbrite public events only behind its capability, use, actor, and price gates', () => {
    process.env.NODE_ENV = 'production'
    process.env.GTM_APIFY_ENABLED = 'true'
    process.env.GTM_APIFY_TOKEN = 'synthetic-test-token'
    process.env.GTM_APIFY_CUSTOMER_USE_APPROVED = 'true'
    ...
    process.env.GTM_APIFY_TERMS_VERSION = APIFY_REQUIRED_TERMS_VERSION
    process.env.GTM_APIFY_PRICE_VERSION = APIFY_REQUIRED_PRICE_VERSION
    process.env.GTM_APIFY_EVENTBRITE_OPPORTUNITY_ENABLED = 'true'
    process.env.GTM_APIFY_EVENTBRITE_OPPORTUNITY_USE_APPROVED = 'true'

    expect(Object.keys(sourceAdapterRegistry())).not.toContain(
      APIFY_EVENTBRITE_OPPORTUNITY_CONFIG.adapterId,
    )
    process.env.GTM_APIFY_EVENTBRITE_SEARCH_PRICE_VERSION =
      APIFY_EVENTBRITE_OPPORTUNITY_CONFIG.requiredPriceVersion
    expect(Object.keys(sourceAdapterRegistry())).toContain(
      APIFY_EVENTBRITE_OPPORTUNITY_CONFIG.adapterId,
    )
    process.env.GTM_APIFY_ACTOR_EVENTBRITE_SEARCH = 'another/actor'
    expect(Object.keys(sourceAdapterRegistry())).not.toContain(
      APIFY_EVENTBRITE_OPPORTUNITY_CONFIG.adapterId,
    )
  })
  ...
  it('registers freshness-enforcing Reddit search only behind its exact capability gate', () => {
    process.env.NODE_ENV = 'production'
    process.env.GTM_APIFY_ENABLED = 'true'
    process.env.GTM_APIFY_TOKEN = 'synthetic-test-token'
    process.env.GTM_APIFY_CUSTOMER_USE_APPROVED = 'true'
    ...
    process.env.GTM_APIFY_TERMS_VERSION = APIFY_REQUIRED_TERMS_VERSION
    process.env.GTM_APIFY_PRICE_VERSION = APIFY_REQUIRED_PRICE_VERSION
    process.env.GTM_APIFY_REDDIT_FRESH_OPPORTUNITY_ENABLED = 'true'
    process.env.GTM_APIFY_REDDIT_FRESH_OPPORTUNITY_USE_APPROVED = 'true'

    expect(Object.keys(sourceAdapterRegistry())).not.toContain(
      APIFY_REDDIT_FRESH_OPPORTUNITY_CONFIG.adapterId,
    )
    process.env.GTM_APIFY_REDDIT_FRESH_SEARCH_PRICE_VERSION =
      APIFY_REDDIT_FRESH_OPPORTUNITY_CONFIG.requiredPriceVersion
    expect(Object.keys(sourceAdapterRegistry())).toContain(
      APIFY_REDDIT_FRESH_OPPORTUNITY_CONFIG.adapterId,
    )
    process.env.GTM_APIFY_ACTOR_REDDIT_FRESH_SEARCH = 'another/actor'
    expect(Object.keys(sourceAdapterRegistry())).not.toContain(
      APIFY_REDDIT_FRESH_OPPORTUNITY_CONFIG.adapterId,
    )
  })

  it('registers scoped Reddit API search only behind its exact capability gate', () => {
    process.env.NODE_ENV = 'production'
    process.env.GTM_APIFY_ENABLED = 'true'
    process.env.GTM_APIFY_TOKEN = 'synthetic-test-token'
    process.env.GTM_APIFY_CUSTOMER_USE_APPROVED = 'true'
    ...
    process.env.GTM_APIFY_TERMS_VERSION = APIFY_REQUIRED_TERMS_VERSION
    process.env.GTM_APIFY_PRICE_VERSION = APIFY_REQUIRED_PRICE_VERSION
    process.env.GTM_APIFY_REDDIT_API_OPPORTUNITY_ENABLED = 'true'
    process.env.GTM_APIFY_REDDIT_API_OPPORTUNITY_USE_APPROVED = 'true'

    expect(Object.keys(sourceAdapterRegistry())).not.toContain(
      APIFY_REDDIT_API_OPPORTUNITY_CONFIG.adapterId,
    )
    process.env.GTM_APIFY_REDDIT_API_SEARCH_PRICE_VERSION =
      APIFY_REDDIT_API_OPPORTUNITY_CONFIG.requiredPriceVersion
    expect(Object.keys(sourceAdapterRegistry())).toContain(
      APIFY_REDDIT_API_OPPORTUNITY_CONFIG.adapterId,
    )
    process.env.GTM_APIFY_ACTOR_REDDIT_API_SEARCH = 'another/actor'
    expect(Object.keys(sourceAdapterRegistry())).not.toContain(
      APIFY_REDDIT_API_OPPORTUNITY_CONFIG.adapterId,
    )
  })

  it('registers posted-after Reddit search only behind its exact capability gate', () => {
    process.env.NODE_ENV = 'production'
    process.env.GTM_APIFY_ENABLED = 'true'
    process.env.GTM_APIFY_TOKEN = 'synthetic-test-token'
    process.env.GTM_APIFY_CUSTOMER_USE_APPROVED = 'true'
    ...
    process.env.GTM_APIFY_TERMS_VERSION = APIFY_REQUIRED_TERMS_VERSION
    process.env.GTM_APIFY_PRICE_VERSION = APIFY_REQUIRED_PRICE_VERSION
    process.env.GTM_APIFY_REDDIT_POSTED_AFTER_OPPORTUNITY_ENABLED = 'true'
    process.env.GTM_APIFY_REDDIT_POSTED_AFTER_OPPORTUNITY_USE_APPROVED = 'true'

    expect(Object.keys(sourceAdapterRegistry())).not.toContain(
      APIFY_REDDIT_POSTED_AFTER_OPPORTUNITY_CONFIG.adapterId,
    )
    process.env.GTM_APIFY_REDDIT_POSTED_AFTER_SEARCH_PRICE_VERSION =
      APIFY_REDDIT_POSTED_AFTER_OPPORTUNITY_CONFIG.requiredPriceVersion
    expect(Object.keys(sourceAdapterRegistry())).toContain(
      APIFY_REDDIT_POSTED_AFTER_OPPORTUNITY_CONFIG.adapterId,
    )
    process.env.GTM_APIFY_ACTOR_REDDIT_POSTED_AFTER_SEARCH = 'another/actor'
    expect(Object.keys(sourceAdapterRegistry())).not.toContain(
      APIFY_REDDIT_POSTED_AFTER_OPPORTUNITY_CONFIG.adapterId,
    )
  })

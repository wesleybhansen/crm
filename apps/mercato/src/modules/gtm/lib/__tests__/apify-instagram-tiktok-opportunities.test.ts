...
describe('Apify Instagram and TikTok public-post opportunities', () => {
  ...
  it('routes realtor plays on the realtor contract, generic city plays on the generic one, and emits three frozen lanes', () => {
    const play = {
    ...
    // the generic public-post filter keyed on its own words; a nationwide one is not.
    const diners = {
    ...
    expect(opportunitySourceRouting(diners, APIFY_INSTAGRAM_OPPORTUNITY_CONFIG.adapterId).eligible).toBe(true)
    expect(
      buildOpportunityQueryLanes(diners, APIFY_TIKTOK_OPPORTUNITY_CONFIG.adapterId)[0]!.providerQuery

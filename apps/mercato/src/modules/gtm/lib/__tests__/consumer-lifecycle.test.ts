...
describe('consumer lead lifecycle regression', () => {
  it('sources and qualifies named leads, prepares a grounded draft, and never enters automated execution', async () => {
    ...
    expect(evidence[0].license).toEqual(
      expect.objectContaining({
  ...
  it('sources realtor buyer and seller demand opportunities without creating a recipient or send', async () => {
    const em = new FakeEm()
    const play = await seedPlay(em, {
      marketType: 'b2c',
      geography: 'South Bay, California',
    })
    play.entityUnit = 'opportunities'
    play.signalKind = 'social_engagement'
    play.signal = 'Public buyer and seller intent conversations'
    play.audience = 'People publicly discussing buying or selling a home in the South Bay'
    play.executionEligibility = 'strategy_only'

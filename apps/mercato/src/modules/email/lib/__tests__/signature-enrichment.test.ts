...
describe('enrichContactFromSignature', () => {
  ...
  it('writes the phone and job title encrypted, with the phone lookup hash', async () => {
    ...
    const filled = await enrichContactFromSignature(h.query, orgId, contactId, { phone: '+1 555 010 0100', jobTitle: 'Founder' }, h.encryptRow)
  ...
  it('writes nothing when encryption fails', async () => {
    ...
    await expect(enrichContactFromSignature(h.query, orgId, contactId, { phone: '+1 555 010 0100' }, h.encryptRow)).rejects.toThrow('key service down')

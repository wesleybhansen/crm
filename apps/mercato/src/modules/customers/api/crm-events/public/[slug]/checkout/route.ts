...
import { attendeeEmailHashes, whereAttendeeEmail } from '@/modules/customers/lib/event-attendees'
...
export async function POST(req: Request, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    // Duplicate check
    const existing = await whereAttendeeEmail(knex('event_attendees').where('event_id', event.id), email, await attendeeEmailHashes(email, String(event.tenant_id))).where('status', 'registered').first()

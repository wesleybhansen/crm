...
export const metadata = {
  ...
  persistent: true,
  id: 'payments:payment-failed-notification',
}
...
export default async function handle(payload: PaymentFailedPayload, ctx: ResolverContext) {
  ...
  try {
    ...
    const typeDef = notificationTypes.find((t) => t.type === 'payments.payment.failed')
    if (!typeDef) return
    ...
    const notificationInput = buildNotificationFromType(typeDef, {
      ...
      groupKey: `payment-failed-${payload.transactionId}`,
    })
    ...
    console.error('[payments:payment-failed-notification] Failed:', err instanceof Error ? err.message : err)
  }
}
